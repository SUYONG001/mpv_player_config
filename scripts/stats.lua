-- ==============================================================================
-- stats.lua — mpv 统计信息面板（中文汉化 + 系统资源监控）
-- ==============================================================================
-- 项目来源：
--   https://github.com/yosh-wang/mpv-stats.lua-zh-chinese-translation-/tree/main
--
-- 原始项目：
...
--
-- 功能概述：
--   1. 显示播放器的全部统计信息（文件、视频、音频、显示器、缓存、按键、轨道等）
--   2. 通过"自动翻译模块"将英文界面完整汉化为中文
--   3. 通过"系统统计模块"额外显示 CPU / GPU 占用率（Windows / Linux / macOS）
--
-- 本文件按原版功能保持不变，仅在关键位置补充中文注释，便于阅读理解与后续维护。
-- 完成全量中文注释解析
-- ==============================================================================
...
local mp = require 'mp'            -- mpv 核心 API
local utils = require 'mp.utils'   -- 工具函数（文件、字节、JSON 等）
local input = require 'mp.input'   -- 输入交互（按键过滤搜索时用）

-- ==============================================================================
-- 用户配置区（Options）
-- 可通过 script-opts/stats.conf 覆盖
-- ==============================================================================
local o = {
    -- Default key bindings
    key_page_1 = "1",              -- 第 1 页（默认信息）快捷键
    key_page_2 = "2",              -- 第 2 页（扩展帧耗时）
    key_page_3 = "3",              -- 第 3 页（缓存统计）
    key_page_4 = "4",              -- 第 4 页（按键绑定）
    key_page_5 = "5",              -- 第 5 页（轨道信息）
    key_page_0 = "0",              -- 第 0 页（内部性能）
    -- For pages which support scrolling
    key_scroll_up = "UP",          -- 向上滚动
    key_scroll_down = "DOWN",      -- 向下滚动
    key_search = "/",              -- 搜索按键
    key_exit = "ESC",              -- 退出按键
    scroll_lines = 1,              -- 每次滚动行数

    duration = 4,                  -- 单次显示时长（秒）
    redraw_delay = 1,              -- 切换模式下的重绘间隔（秒）
    ass_formatting = true,         -- 是否启用 ASS 特效渲染
    persistent_overlay = false,    -- 是否让统计面板持续覆盖（不被其他 OSD 覆盖）
    filter_params_max_length = 100,-- 滤镜参数长度超过此值时每行一个
    file_tag_max_length = 128,     -- 文件标签显示的最大字节长度
    file_tag_max_count = 16,       -- 最多显示的标签数量
    show_frame_info = false,       -- 是否显示当前帧信息
    term_clip = true,              -- 终端模式下是否裁剪
    track_info_selected_only = true, -- 轨道信息是否只显示已选轨道
    debug = false,                 -- 调试模式

    -- Graph options and style
    plot_perfdata = false,         -- 是否绘制性能曲线
    plot_vsync_ratio = false,      -- 是否绘制垂直同步比率曲线
    plot_vsync_jitter = false,     -- 是否绘制垂直同步抖动曲线
    plot_cache = true,             -- 是否绘制缓存曲线
    plot_tonemapping_lut = false,  -- 是否绘制色调映射 LUT
    skip_frames = 5,               -- 绘制曲线时跳过的帧数
    global_max = true,             -- 曲线是否使用全局最大值
    flush_graph_data = true,       -- 切换时是否清空曲线数据
    plot_bg_border_color = "0000FF", -- 曲线背景边框色
    plot_bg_color = "262626",      -- 曲线背景色
    plot_color = "FFFFFF",         -- 曲线前景色
    plot_bg_border_width = 1.25,   -- 曲线边框宽度

    -- Text style
    font = "",                     -- 主字体（留空使用默认）
    font_mono = "monospace",       -- 等宽字体（数字对齐用）
    font_size = 20,                -- 字体大小
    font_color = "",               -- 字体颜色
    border_size = 1.65,            -- 边框大小
    border_color = "",             -- 边框颜色
    shadow_x_offset = math.huge,   -- 阴影 X 偏移（huge 表示不启用）
    shadow_y_offset = math.huge,   -- 阴影 Y 偏移
    shadow_color = "",             -- 阴影颜色
    alpha = "11",                  -- 透明度
    vidscale = "auto",             -- 是否跟随视频缩放

    -- Custom header for ASS tags to style the text output.
    -- Specifying this will ignore the text style values above and just
    -- use this string instead.
    custom_header = "",            -- 自定义 ASS 头部样式（设置后会忽略上方样式）

    -- Text formatting
    -- With ASS
    ass_nl = "\\N",                -- ASS 换行符
    ass_indent = "\\h\\h\\h\\h\\h",-- ASS 缩进
    ass_prefix_sep = "\\h\\h",     -- ASS 前缀分隔
    ass_b1 = "{\\b1}",             -- ASS 加粗开始
    ass_b0 = "{\\b0}",             -- ASS 加粗结束
    ass_it1 = "{\\i1}",            -- ASS 斜体开始
    ass_it0 = "{\\i0}",            -- ASS 斜体结束
    -- Without ASS
    no_ass_nl = "\n",              -- 非 ASS 换行
    no_ass_indent = "    ",        -- 非 ASS 缩进
    no_ass_prefix_sep = " ",       -- 非 ASS 前缀分隔
    no_ass_b1 = "\027[1m",         -- 终端加粗开始
    no_ass_b0 = "\027[0m",         -- 终端加粗结束
    no_ass_it1 = "\027[3m",        -- 终端斜体开始
    no_ass_it0 = "\027[0m",        -- 终端斜体结束

    bindlist = "no",               -- 若为 yes，启动时把按键列表打印到终端并退出 mpv
}
...
local pages = {}
local scroll_bound = false      -- 是否已绑定滚动键
local searched_text             -- 搜索关键词
local tm_viz_prev = nil         -- 色调映射可视化之前的状态

-- Save these sequences locally as we'll need them a lot
-- 缓存 ASS 转义序列（避免每次重复调用）
...
local perf_buffers = {}         -- 性能曲线缓冲区表
local process_key_binding       -- 处理按键绑定的函数（后文定义）

-- 属性缓存表（缓存窗口缩放、显示器名称、硬解状态等）
...
-- 若未启用 ASS 或使用 persistent_overlay，则转义文本
...
-- 追加"显示同步"信息（DS、Mistimed、Delayed 等）
...
-- 添加文件基础信息（文件名、标题、时长、章节、版本、大小、缓存等）
...
-- PQ（Perceptual Quantizer）EOTF 转换函数（HDR 亮度换算）
...
-- 添加音频轨道信息（编码、AO、设备、音量、声道、采样率、码率、滤镜等）
...
-- 返回一个记录 vsync-ratio / vsync-jitter 的函数（用于曲线绘制）
...
-- bindlist 模式：启动时把按键列表输出到终端并退出（调试用）
...
-- 来源：https://github.com/yosh-wang/mpv-stats.lua-zh-chinese-translation-/tree/main
-- ============================================================

-- 通用词汇翻译表
-- 说明：优先精确匹配；找不到时进行模糊匹配（键长 >= 4 字符时才会参与，避免破坏英文单词）
...
-- 重写 append_property：拦截属性读取的 prefix/suffix
...
-- 全局状态变量
local cpu_usage = "N/A"          -- CPU 占用率
local gpu_usages = {}            -- GPU 占用率表（按显卡名索引）
local cpu_name = nil             -- CPU 型号
local gpu_names = {}             -- GPU 型号列表
local hw_detected = false        -- 是否已检测过硬件
local stats_refresh_timer = nil  -- 刷新定时器
local update_counter = 0         -- 刷新计数器（控制 GPU 刷新频率）
local sys_lang = nil             -- 系统语言（用于 typeperf 计数器名）
local nvidia_smi_usage = nil     -- nvidia-smi 最新返回值（用于 LUID 归属识别）
...
-- 检测系统语言（缓存结果）
-- 目的：typeperf 的计数器名称在中英文系统下不同，需要区分
...
-- 判断是否为虚拟 GPU（远程桌面、虚拟机、向日葵等间接显示驱动）
...
-- 判断是否为集显（iGPU）
-- 规则：NVIDIA 一定是独显；Intel Arc 是独显；AMD Radeon RX 是独显，
--       其余带 Radeon 的按集显处理；Intel UHD / Iris / HD Graphics 是集显
...
-- 获取 CPU 占用率（跨平台：Windows / Linux / macOS）
...
-- 获取 GPU 占用率（Windows 优先 nvidia-smi，其余平台分别处理）
...
-- 修改 add_file：在原文件信息下方追加 CPU / GPU 占用率
